...
<<<<<<< HEAD
	...
	if (!hDeath){
		...
		/////--------------------------------------------------shield-------------------------------------------------------///
			...
			nonshldcnt++;
			if (nonshldcnt >= 2000)																	///----> non-sheild timer 																	
			{
				nonshldcnt = 0;
				natanshldIndex = 0;
				sheild = false;
...
=======
	...
	iShowImage(0, 0, 800, 500, desertBG);
	iSetColor(24, 67, 233);
	...
	if (jump)
	{
		if (jumping)
		{
			iShowImage(natanCordinateX, natanCordinateY + natanJumpCordinate, 80, 100, natanJumpUpIndex);
		}
		else
		{
			iShowImage(natanCordinateX, natanCordinateY + natanJumpCordinate, 80, 100, natanJumpDownIndex);
		}
	}
	else
	{
		if (!natanStandPos)
		{
			iShowImage(natanCordinateX, natanCordinateY, 105, 110, natanRun[natanRunIndex]);
			standCnt++;
			if (standCnt >= 1500)
			{
				standCnt = 0;
...
void jumpAnimation(){
	if (jump)
	{
		if (jumping)
		{
			natanJumpCordinate += 5;
			if (natanJumpCordinate >= natanCordinateY + JUMPLIMIT)
			{
				jumping = false;
			}
		}
		...
		{
			natanJumpCordinate -= 5;
			if (natanJumpCordinate < natanCordinateY + 0)
			{
				jump = false;
				natanJumpCordinate = 0;
			}
		}
	}
